...
namespace Phexor.SettingMenus
{
    public partial class ColorSettings : UserControl
    {
        ...
        private void ColorSettings_Size(object sender, RoutedEventArgs e)
        {
            try
            {
                MainPanel.Height = this.ActualHeight - 42;
                MainPanel.Width = this.ActualWidth - 42;
            }
        ...
        private void PathInput_Click(object sender, RoutedEventArgs e)
        {
            ShowColorPicker(sender as Button);
        }
        ...
        private void Directorys_Click(object sender, RoutedEventArgs e)
        {
            ShowColorPicker(sender as Button);
        }
        ...
        private void ShowColorPicker(Button button)
        {
            ...
            ColorCodeTextBox.Text = ((SolidColorBrush)button.Background).Color.ToString();
        ...
        private void ApplyColorCode_Click(object sender, RoutedEventArgs e)
        {
            if (_currentButton != null)
            {
                try
                {
                    ...
                    if (_currentButton == PathInput)
                    {
                        SettingsControl.Color1 = ColorCodeTextBox.Text;
                        SettingsControl.SetSettings();
                    }
                    else if (_currentButton == ButtonField)
                    {
                        SettingsControl.Color2 = ColorCodeTextBox.Text;
                        SettingsControl.SetSettings();
                    }
                    else if (_currentButton == Directorys)
                    {
                        SettingsControl.Color3 = ColorCodeTextBox.Text;
                        SettingsControl.SetSettings();
                    }
                    else if (_currentButton == Files)
                    {
                        SettingsControl.Color4 = ColorCodeTextBox.Text;
                        SettingsControl.SetSettings();
                    }
                }
                catch (FormatException)
                {
                    MessageBox.Show("Invalid color code. Please enter a valid color code.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
